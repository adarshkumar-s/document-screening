...
async def run_fast_ocr_pipeline(content: bytes, filename: str, lang: str = "auto",
    ...
    # same land identifiers (survey / khasra / village / case no) as the rest of
    # the system and the Land Intelligence bridge. No AI on the critical path.
...
def enqueue_downstream_tasks(doc_id: str, user: Dict[str, Any], mode: str = "ocr_li") -> None:
    """Record follow-up work on the existing ``ai_tasks`` task table.

    Parcel *matching* is performed synchronously by the upload endpoint using
    the canonical resolver (so the document→parcel link exists immediately and
    the mapping architecture stays the single source of truth). What is recorded
    here is the genuinely non-blocking enrichment that must not slow the OCR
    response:

    * ``AI_FIELD_ENHANCE`` — optional AI re-extraction (no-op when no AI client).
    * ``LAND_INTELLIGENCE`` — a durable marker that Land Intelligence review was
      requested for this upload; the mutation / encumbrance / litigation / risk
      status itself is computed synchronously (bulk) or lazily on read (single),
      so there is no second processing state machine.

    mode:
        "ocr_only" -> AI enhancement marker only; no land-intelligence work.
        "ocr_li"   -> AI enhancement + Land Intelligence review marker.

    This reuses the existing task table only; it does not create a second queue
    and it does not start a competing worker loop. The recorded tasks are drained
    by :func:`drain_doc_tasks`, invoked as a FastAPI ``BackgroundTask`` so the
    ...
    try:
        ...
        # run the enrichment synchronously as a safe fallback so the document
        # is still fully usable.
        try:
            _run_ai_enhance(doc_id)
        except Exception:
            pass
        return
    ...
    try:
        add_task(
            ...
            priority="LOW",
            meta={"doc_id": doc_id, "stage": "ai_enhance", "mode": mode, "uploader": uploader},
        )
        if mode == "ocr_li":
            add_task(
                "LAND_INTELLIGENCE",
                f"Land Intelligence review for {doc_id}",
                "Mutation / Encumbrance / Court cases / Risk review for the resolved parcel.",
                priority="MEDIUM",
                meta={"doc_id": doc_id, "stage": "land_intel", "mode": mode, "uploader": uploader},
            )
    except Exception:
        # Fallback: run AI enhancement synchronously if the task write failed.
        try:
            _run_ai_enhance(doc_id)
        except Exception:
            pass
...
def drain_doc_tasks(doc_id: str) -> None:
    """Process the PENDING downstream tasks recorded for ``doc_id`` on the
    existing ``ai_tasks`` table.

    Invoked as a FastAPI ``BackgroundTask`` from the upload / bulk endpoints so
    the non-blocking enrichment runs after the response has been sent. This is
    deliberately NOT a persistent background worker: there is no second worker
    loop, no competing queue, and no always-on thread that would contend for the
    database or pollute per-request query accounting.
    """
    try:
    ...
    try:
        with _get_db() as db:
            rows = db.execute(
                """SELECT id, task_type FROM ai_tasks
                   WHERE task_type IN ('AI_FIELD_ENHANCE','LAND_INTELLIGENCE')
                     AND record_id=? AND status='PENDING'
    ...
    for row in rows:
        ...
        try:
            with _get_db() as db:
                db.execute(
                    "UPDATE ai_tasks SET status='IN_PROGRESS', updated_at=? WHERE id=? AND status='PENDING'",
                    (time.time(), task_id),
                )
            if task_type == "AI_FIELD_ENHANCE":
                # Optional AI re-extraction; no-op without an AI client. Never
                # changes the document status — only augments ai_decision_support.
                _run_ai_enhance(doc_id)
            elif task_type == "LAND_INTELLIGENCE":
                # Land Intelligence status is computed synchronously for bulk
                # items and lazily on read for single uploads, so there is no
                # additional work here — the marker simply records that review
                # was requested for the resolved parcel.
                pass
            with _get_db() as db:
                db.execute(
