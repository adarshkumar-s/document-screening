...
app = FastAPI(
    title="DILRMP Intelligent Land Record Digitization & Validation System"
)
...
def sync_file_backups(
    record_entry: Optional[dict] = None, audit_entry: Optional[dict] = None
):
  try:
    if record_entry:
      existing = []
    ...
    if not cur.fetchone():
      ...
      conn.execute(
          ...
          (
              ...
              "SYSTEM",
              "INIT",
              "Audit log and secure engine initialized",
              None,
          ),
      )
          ...
          for r in records:
            conn.execute(
                ...
                (
                    ...
                    r["verdict"],
                    r["status"],
                    json.dumps(r.get("languages", ["English"])),
                    r.get("pages", 1),
                    json.dumps(r.get("fields", {})),
                    json.dumps(r.get("validation", {})),
                    r.get("ocr_text", ""),
                    r.get("detected_language", "English"),
                    json.dumps(r.get("original_fields", {})),
...
def log_audit(
    username: str, action: str, detail: str, doc_id: Optional[str] = None
):
  entry = {
      "ts": time.time(),
...
def extract_entities(
    text: str, detected_lang: str = "English", pages: int = 1
) -> Dict[str, Any]:
  ...
  if not fields["owner_name"]["value"]:
    m_hon = re.search(
        r"\b(श्री|श्रीमती|মোহাম্মদ|শ্রী|শ্রীমতি|திரு|திருமதி|Shri|Smt|Mr\.)\s+([^\n,\|]+)",
        text,
    )
    if m_hon and len(m_hon.group(0)) > 4:
      fields["owner_name"] = {
          "value": m_hon.group(0).strip(" \t:|-"),
          "confidence": 0.85,
      }
  ...
  return {
      ...
      "fields": fields,
      "validation": {
          "verdict": "valid" if fields["owner_name"]["value"] else "review",
          "issues": [],
      },
      "ocr_text": text,
  }
...
def get_samples():
  ...
  return {
      "samples": sorted(
          [f for f in os.listdir(samples_dir) if not f.startswith(".")]
      )
  }
...
async def process_sample(name: str, user: dict = Depends(get_current_user)):
  ...
  log_audit(
      user["full_name"], "PROCESS_SAMPLE", f"Sample processed: {name}", doc_id
  )
...
async def process_upload(
  ...
  log_audit(
      user["full_name"],
      "UPLOAD_RECORD",
      f"Uploaded record: {safe_filename}",
      doc_id,
  )
...
def get_dashboard(user: dict = Depends(get_current_user)):
  with get_db() as conn:
    ...
    cur.execute(
        "SELECT COUNT(*) as c FROM documents WHERE status='pending_review'"
    )
  ...
  return {
      ...
      "verified": verified,
      "auto_approved": max(0, total - pending),
...
def verify_document(
    doc_id: str, req: VerifyReq, user: dict = Depends(get_current_user)
):
  with get_db() as conn:
    cur = conn.cursor()
    ...
    for k, v in req.corrections.items():
      if k in fields:
        ...
        if old and old != v:
          conn.execute(
              "INSERT INTO corrections (field_id, wrong, right, count) VALUES"
              " (?, ?, ?, 1) ON CONFLICT(field_id, wrong, right) DO UPDATE SET"
              " count=count+1",
              (k, old, v),
          )
    conn.execute(
        "UPDATE documents SET status='verified', fields=? WHERE id=?",
  ...
  log_audit(
      user["full_name"], "VERIFY_RECORD", f"Verified record #{doc_id}", doc_id
  )
...
def delete_document(doc_id: str, user: dict = Depends(get_current_user)):
  with get_db() as conn:
    ...
    conn.commit()
  log_audit(
      user["full_name"], "DELETE_RECORD", f"Deleted record #{doc_id}", doc_id
  )
  return {"status": "ok"}
...
def get_doc_audit(doc_id: str, user: dict = Depends(get_current_user)):
  with get_db() as conn:
    cur = conn.cursor()
    cur.execute(
        "SELECT * FROM audit WHERE doc_id=? ORDER BY id DESC", (doc_id,)
    )
...
def get_corrections(user: dict = Depends(get_current_user)):
  with get_db() as conn:
    cur = conn.cursor()
    cur.execute(
        "SELECT field_id, wrong, right, count FROM corrections ORDER BY count"
        " DESC"
    )
...
def favicon_ico():
  return FileResponse(
      os.path.join(BASE_DIR, "favicon.svg"), media_type="image/svg+xml"
  )
