...
def _demo_mutation(mut_id: str, scenario: str, *, mutation_no: str, survey: str, village: str,
    ...
    with get_db() as db:
        if db.execute("SELECT 1 FROM land_mutations WHERE id=?", (mut_id,)).fetchone():
            return False
        # mutation_no is UNIQUE across the register: if the number is held by
        # any row this seeder does not own, skip cleanly instead of raising.
        # (Demo seeding must never modify or delete a row it did not create.)
