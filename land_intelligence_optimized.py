...
All DDL is startup/migration work. HTTP requests only execute SELECTs.
"""
...
def ensure_property_indexes()->None:
    """Run during startup/deployment, never during an HTTP request."""
    with get_db() as db:
        for name,column in (("idx_properties_survey","survey_number"),("idx_properties_gat","gat_number"),("idx_properties_khasra","khasra_number"),("idx_properties_subdivision","sub_division"),("idx_properties_village","village"),("idx_properties_taluka","taluka"),("idx_properties_district","district")):
            db.execute(f"CREATE INDEX IF NOT EXISTS {name} ON properties ({column})")
...
def _candidate_rows(fields:Dict[str,Any])->List[Dict[str,Any]]:
    ...
    for key in STRONG_KEYS:
        ...
        if value: clauses.append(f"LOWER(TRIM({key})) = LOWER(TRIM(?))");params.append(value)
    ...
    with get_db() as db:
        return [dict(r) for r in db.execute("SELECT * FROM properties WHERE "+" OR ".join(clauses)+" LIMIT 250",tuple(params)).fetchall()]


def resolve_indexed(fields:Dict[str,Any])->Dict[str,Any]:
    supplied={k:_field(fields,k) for k in IDENTIFIER_KEYS if _field(fields,k)};area_value=_field(fields,"area")
    if not supplied and not area_value:return {"status":"INSUFFICIENT DATA","confidence":0,"matches":[],"reasons":["No property identifiers, location fields, or area were extracted."]}
    try:area_num=float(str(area_value).replace(",","").split()[0]) if area_value else None
    except (TypeError,ValueError):area_num=None
    try:tolerance=float(os.getenv("LAND_AREA_TOLERANCE_HA","0.05"))
    except ValueError:tolerance=.05
    scored=[]
    for row in _candidate_rows(fields):
        score=0.;max_score=0.;reasons=[];missing=[];conflicts=[];strong_conflict=False
        ...
        if max_score>0 and score>0:scored.append((round(score/max_score*100),row,reasons,missing,conflicts,strong_conflict))
    scored.sort(key=lambda x:(x[0],len(x[2])),reverse=True)
    if not scored:return {"status":"NO MATCH","confidence":0,"matches":[],"reasons":["No parcel matched the supplied property identity or location fields."]}
    matches=[]
    for pct,row,reasons,missing,conflicts,_ in scored[:5]:
        if pct>=50:matches.append({"property":_property(row,False),"confidence":pct/100,"reasons":reasons,"missing_fields":missing,"conflicting_fields":conflicts})
