(function(){
  ...
  function nav(){
    ...
    if(!box){
      ...
      const land=document.createElement('a');land.href='/land-intelligence';land.className='btn ghost';land.textContent='Land Intelligence';land.style.cssText='display:block;width:100%;box-sizing:border-box;text-align:left;border:0;padding:9px 10px;font-size:12px;text-decoration:none';m.appendChild(land);
  ...
  function records(){document.querySelectorAll('#simpleDocTable tbody tr,#staffRecordsTable tbody tr').forEach(r=>{if(r.querySelector('.land-intel-file-link'))return;const c=r.querySelectorAll('td');if(c.length<2)return;const id=(c[0].textContent||'').trim().replace(/^#/,'').split(/\s+/)[0];if(!/^\d+$/.test(id))return;const a=document.createElement('a');a.className='btn ghost land-intel-file-link';a.href='/land-intelligence?document_id='+encodeURIComponent(id);a.textContent='Land Intelligence';a.style.cssText='display:inline-block;margin-left:6px;padding:5px 9px;font-size:11px;white-space:nowrap;text-decoration:none';c[c.length-1].appendChild(a)})}
