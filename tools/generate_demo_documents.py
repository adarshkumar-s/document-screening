...
def _build_pdf_with_text(img_bytes: bytes, img_w: int, img_h: int, text_str: str) -> bytes:
    import zlib
    lines = [l for l in text_str.split("\n") if l.strip()]
    text_ops = ["BT", "/F1 12 Tf", "36 750 Td", "14 TL"]
    for l in lines:
        clean = l.replace("\\", "/").replace("(", "[").replace(")", "]")
        text_ops.append(f"({clean}) '")
    text_ops.append("ET")
    content_stream = "q 595 0 0 842 0 0 cm /Im1 Do Q\n" + "\n".join(text_ops)
    ...
    img_obj = f"<</Type /XObject /Subtype /Image /Width {img_w} /Height {img_h} /ColorSpace /DeviceRGB /BitsPerComponent 8 /Filter /DCTDecode /Length {len(img_bytes)}>>\nstream\n".encode("ascii") + img_bytes + b"\nendstream"
    c_compressed = zlib.compress(content_bytes)
    content_obj = f"<</Length {len(c_compressed)} /Filter /FlateDecode>>\nstream\n".encode("ascii") + c_compressed + b"\nendstream"
    add_obj(b"<</Type /Catalog /Pages 2 0 R>>")
    add_obj(b"<</Type /Pages /Kids [3 0 R] /Count 1>>")
    add_obj(b"<</Type /Page /Parent 2 0 R /MediaBox [0 0 595 842] /Contents 6 0 R /Resources <</ProcSet [/PDF /Text /ImageC] /Font <</F1 5 0 R>> /XObject <</Im1 4 0 R>> >> >>")
    add_obj(img_obj)
...
def render_spec(spec: Dict[str, Any], out_dir: str, force: bool = False) -> str:
    ...
    if spec["filename"].lower().endswith(".pdf"):
        ...
        pdf_bytes = _build_pdf_with_text(buf.getvalue(), PAGE_W, PAGE_H, text)
