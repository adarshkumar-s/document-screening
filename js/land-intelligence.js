(() => {
  ...
  const state = {records:[], mutations:[], encumbrances:[], cases:[], risk:[], properties:[], mapRecords:[]};
  ...
  async function renderMap(){
    ...
    try{
      const params=new URLSearchParams({village:'',district:'',survey:'',limit:'1000'});
      const [properties, records] = await Promise.all([api(`/api/map/properties?${params}`),api('/api/map/records?limit=2000')]);
      state.properties=properties.properties||[]; state.mapRecords=records.records||[];
  ...
  $('mutationForm').addEventListener('submit',async e=>{e.preventDefault();const payload=Object.fromEntries(new FormData(e.target).entries());try{await api('/api/mutations',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)});$('mutationDialog').close();e.target.reset();await loadAll();document.querySelector('[data-tab="mutations"]').click();}catch(err){showMessage(err.message);}});
  window.LandIntelligenceOpen=id=>openLand(id);loadAll();
})();
